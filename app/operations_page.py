"""Operations Center UI (sensor ML lab). Shared by multipage and legacy dashboard."""
...
def render_operations() -> None:
    ...
    with st.sidebar:
        ...
        if engine is not None:
            ...
            st.markdown(f"**ML:** `{'READY' if st_status.ready else 'NOT READY'}`")
            st.caption(
    ...
    with d2:
        ...
        motor_color = (
            "#D95C5C"
            if "ELECTRICAL" in result.failure_mode or "OVERHEAT" in result.failure_mode
            else "#4CAF78"
        )
